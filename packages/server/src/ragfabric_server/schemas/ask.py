...
class AskRequest(BaseModel):
    ...
    # Widened in Phase 4 as the vectorless strategy actually shipped. The
    # pattern is still a closed set rather than a free string: a name the
    # server cannot serve must be a 422 from validation, not a KeyError out of
    # the registry surfacing as a 500.
    strategy: str = Field(default="traditional", pattern="^(traditional|vectorless)$")
    stream: bool = True
