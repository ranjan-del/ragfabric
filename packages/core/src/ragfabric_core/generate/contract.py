...
Known looseness, recorded rather than hidden: rule 3 is skipped for any answer
that merely contains the ``NO_EVIDENCE`` substring, wherever it appears. A
model that writes "I could not find a date, but the policy is X" with no
marker would pass, because the substring check cannot tell an admission from
an admission plus an uncited claim. ``NO_EVIDENCE`` is imported by
``generate.cited`` rather than redefined there, so the sentinel the contract
checks for and the sentence the generator emits can never drift apart.
"""
